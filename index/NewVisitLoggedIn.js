async function postNewVisit(userId, userEmail, userName, userNumber) {
    const firstVisitKey = `first_visit_${userId}`;
    const newVisitSessionKey = `new_visit_${userId}`;

    // Verifica se a primeira visita já foi registrada e se o evento new_visit já foi acionado na sessão
    ...
    // Marca o evento `new_visit` como acionado na sessão
    sessionStorage.setItem(newVisitSessionKey, 'true');
}
  
// Chame a função para a nova visita
postNewVisit(properties.param1, properties.param2, properties.param3, properties.param4);
